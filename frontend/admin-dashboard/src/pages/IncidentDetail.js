...
export default function IncidentDetail() {
  ...
  const load = useCallback(async () => {
    try {
      const [incidentResponse, commentResponse, userResponse] = await Promise.all([
        getIncidentById(id),
        getCommentsByIncident(id),
        getAllUsers(),
      ]);
  ...
  const handleStatusChange = async newStatus => {
    try {
      ...
      setError(requestError.response?.data?.message || 'Transition invalide.');
    }
  };
  ...
  const techs = users.filter(user => user.role === 'ROLE_TECHNICIEN');
        ...
        <div>
          <div className="detail-card">
              ...
              {WORKFLOW.map((status, index) => (
                <React.Fragment key={status}>
                  <button
                    ...
                    style={{ cursor: index > currentStep && index === currentStep + 1 ? 'pointer' : 'default' }}
                    onClick={() => index === currentStep + 1 && handleStatusChange(status)}
                  >
                    {status}
                  </button>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>
              Cliquez sur la prochaine etape pour avancer le workflow.
            </p>
          </div>

          <div className="detail-card">
            <div className="detail-card-title">
              <AlertTriangle size={14} /> Details de l'incident
            </div>
            <div className="detail-row">
              ...
              <div className="detail-field">
                <div className="label">Cree le</div>
                <div className="value td-mono" style={{ fontSize: 13 }}>
                  {new Date(incident.createdAt).toLocaleString('fr-FR')}
                </div>
              </div>
              <div className="detail-field">
                <div className="label">Assigne a</div>
                <div className="value">{incident.assignedToKeycloakId || 'Non assigne'}</div>
              </div>
            </div>
            ...
            <div className="form-group">
              ...
              <select value={selectedTech} onChange={event => setSelectedTech(event.target.value)}>
                ...
                {techs.map(technician => (
                  <option key={technician.keycloakId} value={technician.keycloakId}>
                    {technician.username}
                  </option>
                ))}
              </select>
            </div>
            <button className="btn btn-primary" style={{ width: '100%' }} onClick={handleAssign}>
