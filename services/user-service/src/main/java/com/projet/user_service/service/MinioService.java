...
public class MinioService {
    ...
    @Value("${upload.max-file-size-bytes:5242880}")
    ...
    public String uploadAvatar(MultipartFile file, String keycloakId) {
        validateAvatar(file);
        ...
        try {
            ...
            throw new ResponseStatusException(
                    ...
                    "Impossible de stocker l'avatar.",
                    ex
            );
        }
    }

    public String buildAvatarUrl(String objectReference) {
        ...
        try {
            ...
            log.warn("Unable to generate presigned URL for avatar {}", normalizedReference, ex);
            return objectReference;
        }
    }
    ...
    private void validateAvatar(MultipartFile file) {
        if (file == null || file.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "L'avatar est vide.");
        }
        if (file.getSize() > maxFileSizeBytes) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "L'avatar depasse la taille autorisee.");
        }
        if (!ALLOWED_CONTENT_TYPES.contains(file.getContentType())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Le format d'avatar n'est pas autorise.");
        }
    }
    ...
    private String normalizeObjectReference(String objectReference) {
        ...
        try {
            ...
            log.warn("Invalid avatar reference {}", objectReference, ex);
        }
