...
public class MinioService {
    ...
    public String uploadFile(MultipartFile file, String prefix) {
        ...
        try {
            ...
            throw new ResponseStatusException(
                    ...
                    "Impossible de stocker la capture d'ecran.",
                    ex
            );
        }
    }
    ...
    public String buildFileUrl(String objectReference) {
        ...
        try {
            ...
            log.warn("Unable to generate presigned URL for incident object {}", normalizedReference, ex);
            return objectReference;
        }
    }
    ...
    private void validateScreenshot(MultipartFile file) {
        if (file == null || file.isEmpty()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "La capture d'ecran est vide.");
        }
        if (file.getSize() > maxFileSizeBytes) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "La capture d'ecran depasse la taille autorisee.");
        }
        if (!ALLOWED_CONTENT_TYPES.contains(file.getContentType())) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Le format de capture d'ecran n'est pas autorise.");
        }
    }
    ...
    private String normalizeObjectReference(String objectReference) {
        ...
        try {
            ...
            log.warn("Invalid incident object reference {}", objectReference, ex);
        }
